...
# и тогда все сотрудники на два часа заняты их разгрузкой. Во время обеда, разумеется,
...
# без elif и прочего. Решите задание двумя способами:
...
# Обратите внимание на количество условий и постарайтесь сократить их. Не используйте
# перечисление конкретных часов вида a == 1 and a == 2...
# if __name__ == '__main__':
#     time = int(input('Введите время (от 0 до 23): '))
#     if (time >= 8 and time < 10) or (time >= 12 and time < 14) or (time >= 15 and time < 18) or (
#             time >= 20 and time < 22):
...
if __name__ == '__main__':
    time = int(input('Введите время (от 0 до 23): '))
    if (time >= 0 and time < 8) or (time >= 10 and time < 12) or (time >= 14 and time < 15) or (
            time >= 18 and time < 20) or (
            time >= 22 and time <= 24):
        print('Посылку получить нельзя')
    else:
        print('Можно получить посылку')
